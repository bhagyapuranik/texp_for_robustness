"""
This script has been taken from "https://github.com/RobustBench/robustbench" and modified to suit needs.
Replace the original file at "site-packages/robustbench/data.py" with this version and appropriately modify the enums.
"""
import os
...
_clean_dataset_loaders: Dict[BenchmarkDataset, CleanDatasetLoader] = {
    ...
    BenchmarkDataset.tiny_imagenet: load_tiny_imagenet,
...
ZENODO_CORRUPTIONS_LINKS: Dict[BenchmarkDataset, Tuple[str, Set[str]]] = {
    ...
    BenchmarkDataset.tiny_imagenet: ("2469796", {"TinyImageNet-C.tar"})
}
...
CORRUPTIONS_DIR_NAMES: Dict[BenchmarkDataset, str] = {
    ...
    BenchmarkDataset.tiny_imagenet: "TinyImageNet-C",
...
def load_cifar10c(
    ...
    return load_corruptions_cifar(BenchmarkDataset.cifar_10, n_examples,
                                  severity, data_dir, corruptions, shuffle, all_severities)
...
def load_tiny_imagenetc(
    ...
    return load_corruptions_tiny_imagenet(BenchmarkDataset.tiny_imagenet, n_examples,
                                  severity, data_dir, corruptions, shuffle, all_severities)
...
CORRUPTION_DATASET_LOADERS: Dict[BenchmarkDataset, CorruptDatasetLoader] = {
    ...
    BenchmarkDataset.tiny_imagenet: load_tiny_imagenetc,
...
def load_corruptions_cifar(
        ...
        corruptions: Sequence[str] = CORRUPTIONS,
        shuffle: bool = False,
        all_severities: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
    assert 1 <= severity <= 5
    n_total_cifar = 10000

    if all_severities:
        n_examples = n_total_cifar*5
...
def load_corruptions_tiny_imagenet(
        dataset: BenchmarkDataset,
        n_examples: int,
        severity: int,
        data_dir: str,
        ...
        all_severities: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
    assert 1 <= severity <= 5
    n_total_tiny_imagenet = 10000
    
    if all_severities:
        n_examples = n_total_tiny_imagenet*5
    ...
    for corruption in corruptions:
        ...
        if all_severities:
            ...
            images = images_all[(severity - 1) * n_total_tiny_imagenet:severity *
                            n_total_tiny_imagenet]
            n_img = int(np.ceil(n_examples / n_pert))
